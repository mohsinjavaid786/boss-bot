...
export class Store {
   ...
   PRAGMA user_version=1;`);
    if (!this.agents().length) {
      this.addAgent(
        "Atlas",
        "Research & strategy",
        "Produce clear, sourced research. Separate facts from assumptions.",
        "blue",
      );
      this.addAgent(
        "Nova",
        "Writing & communication",
        "Write concise, thoughtful drafts. Never claim a message was sent.",
        "violet",
      );
      this.addAgent(
        "Milo",
        "Operations & planning",
        "Turn broad goals into concrete, actionable plans.",
        "teal",
      );
    }
    for (const task of this.tasks().filter((t) => t.status === "running"))
      this.finish(
        task.id,
        "interrupted",
        "Server restarted during execution. Review before creating a new task.",
      );
  }
  agents(): Agent[] {
    return this.db
  ...
  addAgent(name: string, role: string, instructions: string, color = "blue") {
    const id = randomUUID();
    this.db
      .prepare(
        "INSERT INTO agents(id,name,role,instructions,color) VALUES(?,?,?,?,?)",
      )
      .run(id, name, role, instructions, color);
    return id;
  }
  memory(id: string, memory: string) {
    const result = this.db
      .prepare("UPDATE agents SET memory=? WHERE id=?")
      .run(memory, id);
    if (!result.changes) throw new Error("Agent not found");
  }
  tasks(): Task[] {
    return this.db
      .prepare("SELECT * FROM tasks ORDER BY createdAt DESC,rowid DESC")
  ...
  createTask(agentId: string, prompt: string, runtimeId: string): Task {
    ...
    this.transaction(() => {
      this.db
        .prepare("INSERT INTO tasks VALUES(?,?,?,?,'awaiting_approval','',?,?)")
        .run(id, agentId, prompt, runtimeId, now, now);
  ...
  private event(id: string, event: string) {
    this.db
      .prepare("INSERT INTO events(taskId,event,createdAt) VALUES(?,?,?)")
  ...
  claim(id: string) {
    ...
    this.transaction(() => {
      claimed = !!this.db
        .prepare(
          "UPDATE tasks SET status='running',updatedAt=? WHERE id=? AND status='awaiting_approval'",
        )
        .run(new Date().toISOString(), id).changes;
      if (claimed) this.event(id, "approved");
    });
    return claimed;
  }
  reject(id: string) {
    ...
    this.transaction(() => {
      changed = !!this.db
        .prepare(
          "UPDATE tasks SET status='rejected',updatedAt=? WHERE id=? AND status='awaiting_approval'",
        )
        .run(new Date().toISOString(), id).changes;
      if (changed) this.event(id, "rejected");
    });
    return changed;
  }
  finish(
    ...
    this.transaction(() => {
      const result = this.db
        .prepare(
          "UPDATE tasks SET status=?,output=?,updatedAt=? WHERE id=? AND status='running'",
        )
  ...
  events(id: string) {
    return this.db
      .prepare("SELECT event,createdAt FROM events WHERE taskId=? ORDER BY id")
      .all(id);
  }
  close() {
    this.db.close();
